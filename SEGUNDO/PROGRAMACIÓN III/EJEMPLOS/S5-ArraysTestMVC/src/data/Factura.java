...
public class Factura {
    ...
    public Factura instanceWithRandomAttributes() {
        Factura f = new Factura();
        String[] nifs = {"12345678A", "23456789B","34567890C", "222b"};
        String[] conceptos = {"Recambios", "Fungibles",  "Adquisiciones"};
        String[] razonesSociales = {"Calle del Pez, 13", "Rue du Percebe, 14",
            "Roßauer Lände, 1", "Rebengasse, 3", "Krankenhausstraße, 1"};
        ...
        Random r = new Random();
        f.NIFdelReceptor = nifs[r.nextInt(nifs.length)];
        f.concepto = conceptos[r.nextInt(conceptos.length)];
        f.fecha = fechas[r.nextInt(fechas.length)];
        f.importe = importes[r.nextInt(importes.length)];
        f.nombreReceptor = nombres[r.nextInt(nombres.length)];
        f.direccionDelReceptor = razonesSociales[r.nextInt(razonesSociales.length)];
        f.tipo_de_IVA = ivas[r.nextInt(ivas.length)];
        return f;
    }
   ...
   * Estos son los métodos de escritura y lectura disco
    ...
    public String toStringWithDelimiter(String _delimitador) {
        String resultado;
    ...
    public String toStringAsTableRow() {
        String resultado;
