function [w,coef]=triangular(V,tau,T,tmin,tmax,m)
...
resolucion=pi/1000; %Tiempo entre dos valores consecutivos de la representacion
...
for n=1:m
    w(n+1)=(2*pi*n)/T;
    coef(n+1)=V*tau*(sin((tau*w(n+1))/2)/((tau*w(n+1))/2))^2;
...
figure(1)
plot(t,y, 'r');grid
title('Representacion del tren de pulsos triangulares en el dominio del tiempo');
xlabel('tiempo');
ylabel('voltaje');
