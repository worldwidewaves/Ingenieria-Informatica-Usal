...
int creaVacia(Lista *l){
    if (l == NULL)
        return -2;
    else if (NULL == (l->raiz = l->ultimo = (tipoCeldaLista *)malloc(sizeof(tipoCeldaLista))))
        return -1;
    else {
        l->raiz->sig = NULL;
        l->ultimo->sig = NULL;
...
int inserta(tipoElementoLista x, tipoPosicion p, Lista *l){
    tipoCeldaLista *temp = (tipoCeldaLista *)malloc(sizeof(tipoCeldaLista));
    tipoCeldaLista *aux;
